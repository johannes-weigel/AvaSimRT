...
def test_heightmap_flat_plane(tmp_path: Path) -> None:
    ...
    heightmap_3d, metadata = generate_heightmap(scene_obj, resolution=0.1)
    
    # Extract z-values and coordinates
    ...
    valid_heights = heightmap_z[valid_mask]
    ...
    ax.set_xlabel('X (m)')
    ax.set_ylabel('Y (m)')
    ax.set_title(f'Flat Plane at z=2.0 (resolution={metadata["resolution_m"]}m)')
...
def test_heightmap_pyramid(tmp_path: Path) -> None:
    ...
    assert metadata['heightmap_stats']['z_max'] is not None
    assert metadata['heightmap_stats']['z_max'] > 4.5, "Pyramid peak should be near z=5"
    ...
    ax2.plot_surface(X, Y, heightmap_z, cmap='terrain', alpha=0.9)
    ax2.set_xlabel('X (m)')
    ax2.set_ylabel('Y (m)')
    ...
    contour = ax3.contour(x_coords, y_coords, heightmap_z.T, levels=10, cmap='terrain')
    ...
    print(f"  Height range: [{metadata['heightmap_stats']['z_min']:.2f}, {metadata['heightmap_stats']['z_max']:.2f}]")
...
def test_heightmap_slope_verification(tmp_path: Path) -> None:
    ...
    heightmap_3d, metadata = generate_heightmap(scene_obj, resolution=0.25)
    ...
    for i in range(heightmap_z.shape[0]):
        for j in range(heightmap_z.shape[1]):
            if valid_mask[i, j]:
                x_val = x_coords[i]
                z_val = heightmap_z[i, j]
    ...
    im1 = ax1.imshow(heightmap_z.T, origin='lower', cmap='terrain',
                     extent=[x_coords.min(), x_coords.max(), y_coords.min(), y_coords.max()])
    ax1.set_xlabel('X (m)')
    ax1.set_ylabel('Y (m)')
    ...
    error_map = np.abs(heightmap_z - expected)
    ...
    ax4.plot(x_coords, heightmap_z[:, mid_y_idx], 'o-', label='Computed', markersize=4)
...
def test_heightmap_multiple_resolutions(tmp_path: Path) -> None:
    ...
    for idx, res in enumerate(resolutions):
        heightmap_3d, metadata = generate_heightmap(scene_obj, resolution=res)
        ...
        ax = axes[idx]
        im = ax.imshow(heightmap_z.T, origin='lower',
                       extent=[x_coords.min(), x_coords.max(), y_coords.min(), y_coords.max()])
        ax.set_xlabel('X (m)')
        ax.set_ylabel('Y (m)')
        ax.set_title(f'Resolution: {res}m\n({heightmap_3d.shape[0]}x{heightmap_3d.shape[1]} grid)')
        plt.colorbar(im, ax=ax, label='Height (m)')
